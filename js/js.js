...
// Define the logical video/canvas size in pixels. Keep these in sync with CSS/display size.
const VIDEO_W = 640
const VIDEO_H = 360

// Ensure the canvas internal pixel buffer matches the drawing coordinates
canvas_element.width = VIDEO_W
canvas_element.height = VIDEO_H
...
const update_dots=()=>{
    for(let row=1;row<=flipboard_display_size[1];row++){
        for(let column=1;column<=flipboard_display_size[0];column++){
           // Calculate sampling positions based on the actual canvas/video resolution
           let pixel_information=canvas_context.getImageData(column*(VIDEO_W/flipboard_display_size[0]),row*(VIDEO_H/flipboard_display_size[1]),1,1)
           pixel_information=pixel_information.data[0]+pixel_information.data[1]+pixel_information.data[2]
...
navigator.mediaDevices.getUserMedia({audio:false,video:{width:VIDEO_W,height:VIDEO_H}}).then((videostream)=>{
    video_element.srcObject=videostream
    video_element.play()
    // Draw the video into the canvas at the same logical resolution
    setInterval(()=>{
        canvas_context.drawImage(video_element,0,0,VIDEO_W,VIDEO_H)
    },(1000/15))
})
